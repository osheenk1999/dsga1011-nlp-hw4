...
def create_augmented_dataloader(args, dataset):
    ...
    # 1. Get original training dataset
    train_dataset = dataset["train"]

    # 2. Randomly select 5000 examples to transform
    sampled_dataset = train_dataset.shuffle(seed=42).select(range(5000))

    # 3. Apply your custom transformation to these 5k
    transformed_dataset = sampled_dataset.map(custom_transform, load_from_cache_file=False)

    # 4. Combine original + transformed datasets
    augmented_dataset = datasets.concatenate_datasets([train_dataset, transformed_dataset])

    # 5. Tokenize everything
    tokenized_augmented = augmented_dataset.map(tokenize_function, batched=True, load_from_cache_file=False)

    # 6. Prepare format for model (match existing preprocessing)
    ...
    # 7. Create DataLoader
    train_dataloader = DataLoader(tokenized_augmented, shuffle=True, batch_size=args.batch_size)
